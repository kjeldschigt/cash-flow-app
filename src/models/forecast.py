...
class LoanModel(BaseModel):
    ...
    loan_type: LoanType
    principal_amount: Decimal = Field(..., gt=0, description="Principal amount must be positive")
    interest_rate: Decimal = Field(..., ge=0, le=1, description="Interest rate as decimal (0.05 = 5%)")
    ...
    def calculate_monthly_payment(cls, v, values):
        ...
        if v is None and all(k in values for k in ['principal_amount', 'interest_rate', 'term_months']):
