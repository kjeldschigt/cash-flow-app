...
class BusinessRuleValidator:
    ...
    def validate_payment_schedule(
        ...
        same_date_payments = [
            ...
            if p.get('due_date') == due_date
        ]
        
        total_same_date = sum(
            Decimal(str(p.get('amount', 0))) 
            for p in same_date_payments
        ) + amount
    ...
    def validate_currency_consistency(
        ...
        unusual_currencies = [
            curr for curr in currencies.keys() 
    ...
    def validate_integration_config(
        ...
        missing_fields = [
            field for field in type_requirements 
