...
class SecretVersion:
    ...
    def __init__(
        self, version_id: str, created_date: datetime, is_current: bool = False
    ):
...
class SecretsManager:
    ...
    def get_secret(
        ...
        # Cache the result
        if secret_value:
            self.cache[cache_key] = {"value": secret_value, "timestamp": datetime.now()}
            ...
            if e.response["Error"]["Code"] == "ResourceNotFoundException":
                ...
                try:
                    self.client.create_secret(
                        ...
                        or f"Created on {datetime.now().isoformat()}",
                    )
                    return True
    ...
    def rotate_secret(self, secret_name: str, new_value: str) -> bool:
        ...
        try:
            # Set new version
            if self.set_secret(
                secret_name, new_value, f"Rotated on {datetime.now().isoformat()}"
            ):
    ...
    def _get_azure_secret_versions(self, secret_name: str) -> List[SecretVersion]:
        ...
        try:
            versions = []
            secret_versions = self.client.list_properties_of_secret_versions(
                secret_name
            )
    ...
    def _clear_cache(self, secret_name: str) -> None:
        ...
        keys_to_remove = [
            key for key in self.cache.keys() if key.startswith(f"{secret_name}:")
        ]
    ...
    def emergency_revoke_all(self) -> Dict[str, bool]:
        ...
        for secret_name in secrets:
            try:
                ...
                logger.error(
                    f"Error during emergency revocation of {secret_name}: {str(e)}"
                )
