...
class ServiceIntegrationsUI:
    ...
    def __init__(self):
        ...
        self.vault_service = get_key_vault_service(
            session_id=session_token, user_id=current_user.id if current_user else None
        )
    ...
    def _render_service_tabs(self, current_services: Dict):
        ...
        for idx, (service_key, config) in enumerate(SERVICE_CONFIGS.items()):
            with tabs[idx]:
                self._render_service_config(
                    service_key, config, current_services.get(service_key)
                )
    ...
    def _render_service_config(self, service_key: str, config: Dict, current_key=None):
        ...
        with col1:
            if current_key:
                ...
                if st.button(
                    f"🧪 Test {config['name']} Connection", key=f"test_{service_key}"
                ):
        ...
        with col2:
            if current_key:
                if st.button(
                    f"🗑️ Remove", key=f"remove_{service_key}", type="secondary"
                ):
    ...
    def _render_add_key_form(self, service_key: str, config: Dict):
        ...
        with st.form(f"add_{service_key}_key"):
            ...
            with col1:
                test_before_save = st.checkbox(
                    "Test connection before saving", value=True
                )
            with col2:
                submitted = st.form_submit_button(
                    f"💾 Save {config['name']} Key", type="primary"
                )

            if submitted:
                self._handle_add_key(
                    service_key, key_name, api_key, description, test_before_save
                )
    ...
    def _render_update_key_form(self, service_key: str, config: Dict, current_key):
        ...
        with st.form(f"update_{service_key}_key"):
            ...
            st.info(
                f"Current key: {current_key.masked_value} (Added: {current_key.created_at.strftime('%Y-%m-%d %H:%M')})"
            )
            ...
            new_description = st.text_area(
                "Description", value=current_key.description or "", height=80
            )

            # Options
            col1, col2 = st.columns(2)
            with col1:
                test_before_update = st.checkbox(
                    "Test connection before updating", value=True
                )
            with col2:
                submitted = st.form_submit_button(
                    f"🔄 Update {config['name']} Key", type="primary"
                )
    ...
    def _handle_add_key(
        ...
        if test_before_save:
            with st.spinner(
                f"Testing {SERVICE_CONFIGS[service_key]['name']} connection..."
            ):
                ...
                if temp_success:
                    test_success, test_message, test_details = (
                        self.vault_service.test_api_key(
    ...
    def _handle_update_key(
        self,
        service_key: str,
        key_name: str,
        new_api_key: str,
        new_description: str,
        test_before_update: bool,
    ):
        ...
        if new_api_key and test_before_update:
            with st.spinner(
                f"Testing new {SERVICE_CONFIGS[service_key]['name']} connection..."
            ):
                # Test with temporary key
                temp_key_name = (
                    f"temp_update_{service_key}_{int(datetime.now().timestamp())}"
                )
                temp_success, _ = self.vault_service.store_api_key(
                ...
                if temp_success:
                    test_success, test_message, _ = self.vault_service.test_api_key(
                        temp_key_name, ip_address=ip_address, user_agent=user_agent
                    )

                    # Clean up temporary key
                    self.vault_service.delete_api_key(
                        temp_key_name, ip_address, user_agent
                    )
        ...
        if new_api_key:
            ...
            with self.vault_service.retrieve_api_key(
                key_name, ip_address, user_agent
            ) as key_context:
                if key_context:
                    success, message = self.vault_service.update_api_key(
                        key_name=key_name,
    ...
    def _test_service_connection(self, service_key: str, key_name: str):
        ...
        with st.spinner(
            ...
            success, message, details = self.vault_service.test_api_key(
                key_name, ip_address=ip_address, user_agent=user_agent
            )
    ...
    def _confirm_remove_key(self, service_key: str, key_name: str):
        ...
        st.warning(
            f"⚠️ Are you sure you want to remove the {SERVICE_CONFIGS[service_key]['name']} API key?"
        )
        ...
        with col1:
            if st.button(
                "🗑️ Yes, Remove",
                key=f"confirm_remove_yes_{service_key}",
                type="secondary",
            ):
                ip_address = st.session_state.get("client_ip")
                user_agent = st.session_state.get("user_agent")
                ...
                if success:
                    st.success(
                        f"✅ {SERVICE_CONFIGS[service_key]['name']} integration removed"
                    )
                    st.rerun()
                else:
                    st.error(f"❌ {message}")
