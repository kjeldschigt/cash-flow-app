...
class EnhancedServiceIntegrationsUI:
    ...
    def render_source_priority_info(self):
        ...
        with st.expander("📋 API Key Source Priority System", expanded=False):
            ...
            for service, env_vars in priority_info["environment_mappings"].items():
                ...
                st.markdown(
                    f"**{config.get('icon', '🔧')} {config.get('name', service.title())}**: `{', '.join(env_vars)}`"
                )
    ...
    def render_service_overview(self):
        ...
        for idx, (service_key, resolved_key) in enumerate(resolved_keys.items()):
            ...
            with col:
                # Service header
                st.markdown(
                    f"### {config.get('icon', '🔧')} {config.get('name', service_key.title())}"
                )
                ...
                # Quick actions
                if st.button(
                    f"Configure {config.get('name', service_key)}",
                    key=f"config_{service_key}",
                ):
    ...
    def render_service_configuration(self, service_key: str):
        ...
        resolved_key = self.resolver.resolve_api_key(
            f"{service_key}_api_key", service_key
        )

        st.markdown(
            f"## {config.get('icon', '🔧')} {config.get('name', service_key.title())} Configuration"
        )
        ...
        with col1:
            ...
            if resolved_key.is_valid:
                st.success(
                    f"✅ **Connected** via {get_source_badge(resolved_key.source)}"
                )
        ...
        tab1, tab2, tab3 = st.tabs(
            ["🗄️ Database (UI)", "🌍 Environment", "☁️ Streamlit Cloud"]
        )
    ...
    def _render_database_config(
        self, service_key: str, config: Dict, resolved_key: ResolvedAPIKey
    ):
        """Render database configuration section"""
        st.markdown("**Store API key in encrypted database (Recommended)**")

        # Check if key exists in database
        db_key = self.resolver._check_database_source(
            f"{service_key}_api_key", service_key
        )
        ...
        if db_key is None or st.session_state.get(f"update_mode_{service_key}", False):
            with st.form(f"add_key_form_{service_key}"):
                ...
                if submitted and new_key:
                    self._handle_database_key_save(
                        service_key, new_key, description, test_before_save
                    )

        # Remove confirmation
        if st.session_state.get(f"confirm_remove_{service_key}", False):
            st.warning(
                "⚠️ Are you sure you want to remove this API key from the database?"
            )
            col1, col2 = st.columns(2)
            with col1:
    ...
    def _render_environment_config(self, service_key: str, config: Dict):
        ...
        if env_key:
            ...
            st.info(
                f"**Masked Value**: `{env_key[:4]}...{env_key[-4:] if len(env_key) > 8 else '***'}`"
            )
        else:
        ...
        for env_var in env_vars:
            status = (
                "✅"
                if env_key and env_var in [k for k in env_vars if os.getenv(k)]
                else "❌"
            )
...
{env_vars[0] if env_vars else 'API_KEY'}=your_api_key_here
    ...
    def _render_streamlit_secrets_config(self, service_key: str, config: Dict):
        ...
        if secrets_key:
            ...
            st.info(
                f"**Masked Value**: `{secrets_key[:4]}...{secrets_key[-4:] if len(secrets_key) > 8 else '***'}`"
            )
        else:
        ...
        with st.expander("How to set Streamlit secrets"):
            ...
            secrets_example = "\n".join(
                [f'{key} = "your_api_key_here"' for key in secrets_keys[:2]]
            )
            st.code(
                f"""
...
{secrets_example}
    ...
    def _handle_database_key_save(
        self, service_key: str, api_key: str, description: str, test_first: bool
    ):
        ...
        try:
            if hasattr(self.resolver, "vault_service") and self.resolver.vault_service:
                success, message = self.resolver.vault_service.store_api_key(
                    ...
                    or f"{SERVICE_CONFIGS[service_key]['name']} API key",
                )

                if success:
                    st.success(
                        f"✅ {SERVICE_CONFIGS[service_key]['name']} API key saved successfully"
                    )
    ...
    def _handle_database_key_removal(self, service_key: str):
        ...
        try:
            if hasattr(self.resolver, "vault_service") and self.resolver.vault_service:
                ...
                if success:
                    st.success(
                        f"✅ {SERVICE_CONFIGS[service_key]['name']} API key removed"
                    )
