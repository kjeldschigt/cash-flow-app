...
class APIKeyManagementUI:
    ...
    def _render_api_keys_list(self):
        ...
        if not api_keys:
            st.info(
                "No API keys found. Add your first API key using the 'Add Key' tab."
            )
            return
            ...
            with st.container():
                ...
                with col4:
                    if api_key.is_active:
                        if st.button(
                            "🗑️ Delete", key=f"delete_{api_key.id}", type="secondary"
                        ):
        ...
        with st.form("add_api_key_form", clear_on_submit=True):
            ...
            if submitted:
                self._handle_add_api_key(
                    key_name, api_key, service_type, description, test_before_save
                )
        ...
        if test_before_save:
            with st.spinner("Testing API key connection..."):
                ...
                if not success:
                    ...
                    st.info(
                        "You can still save the key by unchecking 'Test connection before saving'"
                    )
                    return
                else:
    ...
    def _confirm_delete_api_key(self, api_key: APIKeyInfo):
        ...
        st.warning(
            f"⚠️ Are you sure you want to delete the API key '{api_key.key_name}'?"
        )
        ...
        with col1:
            if st.button(
                "🗑️ Yes, Delete", key=f"confirm_yes_{api_key.id}", type="secondary"
            ):
                success, message = self.api_key_service.delete_api_key(api_key.key_name)
                if success:
                    st.success(f"✅ {message}")
                    logger.info(
                        "API key deleted via UI",
