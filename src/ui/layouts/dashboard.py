...
class DashboardLayout:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "theme": "light",
            "sections": [
                {"name": "overview", "title": "Financial Overview", "widgets": []},
                {"name": "metrics", "title": "Key Metrics", "widgets": []},
                {"name": "charts", "title": "Analytics", "widgets": []},
            ],
        }
    ...
    def render(self):
        ...
        st.set_page_config(
            ...
            initial_sidebar_state=(
                "expanded" if self.layout_config.get("sidebar_enabled") else "collapsed"
            ),
        )
    ...
    def _render_section(self, section: Dict[str, Any]):
        ...
        visible_widgets = [
            w
            for w in widgets
            if w.get("id", w.get("title", ""))
            not in self.user_preferences.get("hidden_widgets", [])
        ]
...
class WidgetManager:
    ...
    def create_chart_widget(
        ...
        return {
            ...
            "type": "chart",
            "title": title,
            "config": {"chart_type": chart_type, "data": data, **(chart_config or {})},
        }
    ...
    def create_table_widget(
        ...
        return {
            ...
            "type": "table",
            "title": title,
            "config": {"data": data, **(table_config or {})},
        }
    ...
    def create_custom_widget(
        ...
        return {
            ...
            "type": "custom",
            "title": title,
            "config": {"render_function": render_function, "params": params or {}},
        }
    ...
    def render_sortable_widgets(self, widgets: List[Dict[str, Any]], section_name: str):
        ...
        if len(widget_names) > 1:
            ...
            with st.expander("🔧 Reorder Widgets"):
                ...
                if st.button(
                    f"Apply Order to {section_name}", key=f"apply_{section_name}"
                ):
...
class ThemeManager:
    ...
    def get_theme_selector():
        ...
        return st.selectbox(
            "Dashboard Theme", list(ThemeManager.THEMES.keys()), key="theme_selector"
        )
        ...
        "sections": [
            {
                ...
                "widgets": [
                    ...
                    WidgetManager.create_metric_widget(
                        ...
                        "Net Profit",
                        data.get("profit", 0),
                        data.get("profit_delta"),
                    ),
                    WidgetManager.create_metric_widget(
                        "margin",
                        "Margin",
                        "Profit Margin",
                        f"{data.get('margin', 0):.1f}%",
                        data.get("margin_delta"),
                    ),
                ],
            },
            {
