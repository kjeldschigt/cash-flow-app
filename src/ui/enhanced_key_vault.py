...
class EnhancedKeyVaultUI:
    ...
    def render_key_vault_management(self):
        ...
        st.markdown(
            "Enterprise-grade API key management with encryption and audit logging"
        )
        ...
        tab1, tab2, tab3, tab4 = st.tabs(
            ["📋 Vault Keys", "➕ Store Key", "📊 Vault Status", "📜 Audit Logs"]
        )
    ...
    def _render_vault_keys_list(self):
        ...
        api_keys = self.vault_service.list_api_keys(
            service_type=service_type, include_inactive=show_inactive
        )

        if not api_keys:
            st.info(
                "No API keys found in vault. Store your first key using the 'Store Key' tab."
            )
            return
        ...
        for api_key in api_keys:
            with st.container():
                ...
                with col2:
                    ...
                    st.caption(
                        f"Added: {api_key.created_at.strftime('%Y-%m-%d %H:%M')}"
                    )
                ...
                with col4:
                    if api_key.is_active:
                        if st.button(
                            "🗑️ Delete",
                            key=f"vault_delete_{api_key.id}",
                            type="secondary",
                        ):
    ...
    def _render_store_key_form(self):
        ...
        with st.form("store_vault_key_form", clear_on_submit=True):
            ...
            with col1:
                test_before_store = st.checkbox(
                    "Test connection before storing", value=True
                )
            ...
            if submitted:
                self._handle_store_key(
                    key_name, api_key, service_type, description, test_before_store
                )
    ...
    def _render_vault_status(self):
                ...
                with st.expander(f"🔑 {key_name}"):
                    ...
                    with col2:
                        ...
                        if key_stats["last_accessed"]:
                            st.write(
                                f"**Last Access:** {key_stats['last_accessed'][:19]}"
                            )
        ...
        if api_keys:
            ...
            for service_type, keys in services.items():
                with st.expander(
                    f"📡 {service_type.replace('_', ' ').title()} ({len(keys)} keys)"
                ):
                    for api_key in keys:
                        col1, col2, col3 = st.columns([2, 1, 1])
                        ...
                        with col2:
                            st.caption(
                                f"Added: {api_key.created_at.strftime('%Y-%m-%d')}"
                            )
    ...
    def _handle_store_key(
        self,
        key_name: str,
        api_key: str,
        service_type: str,
        description: str,
        test_before_store: bool,
    ):
            ...
            with st.spinner("Testing API key connection..."):
                ...
                if temp_success:
                    ...
                    if not test_success:
                        ...
                        st.info(
                            "You can still store the key by unchecking 'Test connection before storing'"
                        )
                        return
                    else:
    ...
    def _test_vault_key(self, api_key: APIKeyInfo):
        ...
        with st.spinner(f"Testing connection for {api_key.key_name}..."):
            success, message, details = self.vault_service.test_api_key(
                api_key.key_name, ip_address=ip_address, user_agent=user_agent
            )
    ...
    def _handle_update_vault_key(
        self, api_key: APIKeyInfo, new_api_key: str, new_description: str
    ):
        ...
        if new_api_key:
            ...
            with self.vault_service.retrieve_api_key(
                api_key.key_name, ip_address, user_agent
            ) as key_context:
                if key_context:
                    success, message = self.vault_service.update_api_key(
                        api_key.key_name,
                    ...
                    success, message = (
                        False,
                        "Failed to retrieve current key for update",
                    )
    ...
    def _confirm_delete_vault_key(self, api_key: APIKeyInfo):
        ...
        st.warning(
            f"⚠️ Are you sure you want to delete the vault key '{api_key.key_name}'?"
        )
        ...
        with col1:
            if st.button(
                "🗑️ Yes, Delete", key=f"vault_confirm_yes_{api_key.id}", type="secondary"
            ):
                ip_address = st.session_state.get("client_ip")
                user_agent = st.session_state.get("user_agent")
