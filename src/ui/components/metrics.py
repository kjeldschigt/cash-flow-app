    ...
    def render(
        ...
        if format_value and isinstance(value, (int, float)):
            if abs(value) >= 1_000_000:
                ...
                formatted_value = (
                    f"{value:,.0f}" if isinstance(value, int) else f"{value:,.2f}"
                )
        else:
...
class TrendIndicator:
    ...
    def render(
        ...
        with col2:
            st.markdown(
                f"<span style='font-size: 24px;'>{arrow}</span>", unsafe_allow_html=True
            )
    ...
    def render(
        ...
        with col4:
            ProfitMarginCard.render(
                margin=margin, previous_margin=previous_margin, period=period
            )
...
def render_revenue_metrics(
    data: Dict[str, Any], currency: str = "USD", period: str = "month"
):
...
def render_cost_breakdown_metrics(
    data: Dict[str, Any], currency: str = "USD", period: str = "month"
):
