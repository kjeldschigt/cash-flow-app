            ...
            with st.expander("📈 Forecast Summary"):
                ...
                with col2:
                    if len(forecast_data) > 1:
                        growth_rate = ((forecast_data[value_column].iloc[-1] / forecast_data[value_column].iloc[0]) - 1) * 100
                        st.metric("Growth Rate", f"{growth_rate:.1f}%")
                
                with col3:
                    forecast_range = forecast_data[value_column].max() - forecast_data[value_column].min()
            ...
            for scenario_name, data in scenarios.items():
                if not data.empty:
                    comparison_data[scenario_name] = {
                        ...
                        'Final Value': data[value_column].iloc[-1] if len(data) > 0 else 0
                    }
...
def render_monthly_trends(data: pd.DataFrame, metrics: List[str], title: str = "Monthly Trends"):
