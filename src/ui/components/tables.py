...
class BaseTable:
    ...
    def _prepare_dataframe(
        df: pd.DataFrame, formatters: Dict[str, Callable] = None
    ) -> pd.DataFrame:
...
class TransactionTable:
    ...
    def render(
                    ...
                    with filter_cols[col_idx]:
                        if filtered_data[column].dtype in ["object", "category"]:
                            ...
                            unique_values = ["All"] + sorted(
                                filtered_data[column].dropna().unique().tolist()
                            )
        ...
        if total_pages > 1:
            ...
            with col2:
                page = st.selectbox(
                    "Page", range(1, total_pages + 1), key=f"page_{title}"
                )
...
class CostBreakdownTable:
    ...
    def render(
        ...
        category_totals = (
            data.groupby(category_column)[amount_column]
            .agg(["sum", "count", "mean"])
            .reset_index()
        )
        ...
        if show_subcategories:
            ...
            if selected_category != "All Categories":
                ...
                if "subcategory" in category_data.columns:
                    subcategory_totals = (
                        category_data.groupby("subcategory")[amount_column]
                        .agg(["sum", "count"])
                        .reset_index()
                    )
                    ...
                    subcategory_totals["Total Amount"] = subcategory_totals[
                        "Total Amount"
        ...
        BaseTable._add_export_functionality(
            category_totals, f"{export_filename}_summary"
        )
    ...
    def render(
        ...
        if editable_scenarios:
            ...
            with col2:
                if st.button("Create Scenario"):
                    if (
                        scenario_name
                        and scenario_name not in st.session_state.forecast_scenarios
                    ):
                        st.session_state.forecast_scenarios[scenario_name] = (
                            forecast_data.copy()
                        )
        ...
        if st.session_state.forecast_scenarios:
            scenario_tabs = st.tabs(
                ["Base Forecast"] + list(st.session_state.forecast_scenarios.keys())
            )
            ...
            for i, (scenario_name, scenario_data) in enumerate(
                st.session_state.forecast_scenarios.items()
            ):
                with scenario_tabs[i + 1]:
                    if editable_scenarios:
                        ...
                        if not edited_scenario.equals(forecast_data):
                            ...
                            numeric_cols = edited_scenario.select_dtypes(
                                include=["number"]
                            ).columns
                            variance_data = {}
                            ...
                            for col in numeric_cols:
                                if col in forecast_data.columns:
                                    ...
                                    variance_pct = (
                                        variance / forecast_data[col] * 100
                                    ).fillna(0)
            ...
            if editable_scenarios:
                edited_forecast = st.data_editor(
                    forecast_data, use_container_width=True, key=f"base_forecast_editor"
                )
            ...
            if st.button("📊 Export All Scenarios"):
                ...
                with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                    forecast_data.to_excel(
                        writer, sheet_name="Base_Forecast", index=False
                    )

                    for (
                        scenario_name,
                        scenario_data,
                    ) in st.session_state.forecast_scenarios.items():
                        safe_name = scenario_name.replace(" ", "_")[
                            :30
                        ]  # Excel sheet name limit
                        scenario_data.to_excel(
    ...
    def render(
        ...
        if index_cols and value_cols:
            try:
                ...
                # Export functionality
                BaseTable._add_export_functionality(
                    pivot_data.reset_index(), export_filename
                )
            ...
            st.info(
                "Please select at least one index column and one value column to create the pivot table."
            )


# Convenience functions for common table patterns
def render_financial_summary_table(
    data: Dict[str, pd.DataFrame], title: str = "Financial Summary"
):
...
def render_comparison_table(
    ...
    for col in numeric_cols:
        if col in data2.columns:
            ...
            variance_pct = (
                (variance / data2[col].sum() * 100) if data2[col].sum() != 0 else 0
            )

            comparison_data[f"{col} Variance"] = (
                f"${variance:,.2f}" if "amount" in col.lower() else f"{variance:,.2f}"
            )
