...
from ui.components.charts import (
    ...
    render_kpi_sparklines,
)
from ui.components.tables import TransactionTable, render_financial_summary_table
from ui.layouts.dashboard import (
    DashboardLayout,
    WidgetManager,
    create_financial_overview_layout,
)
...
class DashboardPage:
    ...
    def render(self, data_manager=None):
        ...
        """
        st.set_page_config(
            page_title=self.page_title, page_icon=self.page_icon, layout="wide"
        )
    ...
    def _load_dashboard_data(self, data_manager) -> Dict[str, Any]:
        ...
        try:
            ...
            total_cash_out = (
                cash_out_data["amount"].sum() if not cash_out_data.empty else 0
            )
        ...
        sample_costs = pd.DataFrame(
            {
                ...
                "category": [
                    ["Operating", "Marketing", "Personnel", "Other"][i % 4]
                    for i in range(100)
                ],
            }
        )
        ...
        sample_sales = pd.DataFrame(
            {
                ...
                "category": [
                    ["Product Sales", "Service Revenue", "Subscriptions"][i % 3]
                    for i in range(80)
                ],
            }
        )
    ...
    def _render_overview_section(self, data: Dict[str, Any]):
        ...
        FinancialSummaryCard.render(
            revenue=data["revenue"], costs=data["costs"], currency="USD", period="month"
        )
    ...
    def _render_cash_flow_section(self, data: Dict[str, Any]):
        ...
        if not data["sales_data"].empty and not data["cash_out_data"].empty:
            ...
            cash_flow_data = self._prepare_cash_flow_data(
                data["sales_data"], data["cash_out_data"]
            )
    ...
    def _render_analytics_section(self, data: Dict[str, Any]):
        ...
        with col1:
            ...
            if not data["sales_data"].empty:
                revenue_breakdown = (
                    data["sales_data"].groupby("category")["amount"].sum().to_dict()
                )

                RevenueBreakdownPieChart.render(
                    data=revenue_breakdown, title="Revenue by Category"
                )
            else:
        ...
        with col2:
            ...
            if not data["costs_data"].empty:
                cost_breakdown = (
                    data["costs_data"].groupby("category")["amount"].sum().to_dict()
                )

                RevenueBreakdownPieChart.render(
                    data=cost_breakdown, title="Costs by Category"
                )
            else:
        ...
        if not data["costs_data"].empty:
            ...
            render_monthly_trends(
                monthly_data, ["Revenue", "Costs", "Profit"], "Monthly Financial Trends"
            )
    ...
    def _prepare_cash_flow_data(
        self, sales_data: pd.DataFrame, cash_out_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        # Merge data
        cash_flow = pd.merge(
            sales_daily, cash_out_daily, on="date", how="outer"
        ).fillna(0)
    ...
    def _prepare_monthly_trends(self, data: Dict[str, Any]) -> pd.DataFrame:
        ...
        if not data["sales_data"].empty:
            sales_monthly = (
                data["sales_data"].set_index("date").resample("M")["amount"].sum()
            )
        else:
        ...
        if not data["costs_data"].empty:
            costs_monthly = (
                data["costs_data"].set_index("date").resample("M")["amount"].sum()
            )
        else:
        ...
        monthly_data = pd.DataFrame(
            {"Revenue": sales_monthly, "Costs": costs_monthly}
        ).fillna(0)
