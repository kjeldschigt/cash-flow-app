...
class EnhancedAuthComponents:
        ...
        with st.form("login_form"):
            ...
            if submitted:
                ...
                if not user:
                    ...
                    logger.warning(
                        "Failed login attempt", email=email, operation="show_login_form"
                    )
                    return False
                ...
                success = self.session_middleware.login_user(
                    user, client_info["ip_address"], client_info["user_agent"]
                )
    ...
    def show_user_info(self):
        ...
        with col2:
            if session_info.get("created_at"):
                ...
                st.write(
                    f"**Session Started:** {created_at.strftime('%Y-%m-%d %H:%M:%S')}"
                )

            if session_info.get("last_accessed"):
                last_accessed = datetime.fromisoformat(session_info["last_accessed"])
                st.write(
                    f"**Last Active:** {last_accessed.strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def show_session_management(self):
        ...
        user_id = st.text_input(
            "User ID", placeholder="Enter user ID to manage sessions"
        )
        ...
        with col1:
            if st.button("Get Session Info"):
                if user_id:
                    info = self.session_middleware.session_manager.get_session_info(
                        user_id
                    )
                    st.json(info)
        ...
        with col2:
            if st.button("Invalidate All Sessions"):
                if user_id:
                    count = self.session_middleware.session_manager.invalidate_all_user_sessions(
                        user_id
                    )
        ...
        with col3:
            if st.button("Check Rate Limits"):
                if user_id and self.session_middleware.rate_limiter:
                    status = self.session_middleware.rate_limiter.get_rate_limit_status(
                        user_id, "auth_login"
                    )
                    st.json(status)
    ...
    def require_exact_role(self, required_role: UserRole) -> bool:
        ...
        if user_role != required_role:
            st.error(
                f"❌ {required_role.value.title()} role required. You have {user_role.value} role."
            )
            st.stop()
            return False
