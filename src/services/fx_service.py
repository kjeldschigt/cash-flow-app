...
class FXService:
    ...
    def convert_crc_to_usd(
        self, amount_crc: Decimal, month: str, rate_type: str = "base"
    ) -> Decimal:
    ...
    def add_fx_rate(
        self, month: str, low_rate: Decimal, base_rate: Decimal, high_rate: Decimal
    ) -> bool:
        ...
        with self.db.get_connection() as conn:
            ...
            conn.execute(
                query, (month, float(low_rate), float(base_rate), float(high_rate))
            )
            return True
...
def get_rate_scenarios(month: str = None):
    ...
    return {"low": 0.85, "base": 0.90, "high": 0.95}
