...
class APIKeyTestService:
    ...
    def test_stripe_key(self, api_key: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    "Stripe API key test successful",
                    operation="test_stripe_key",
                    status_code=response.status_code,
                )
                return (
                    True,
                    f"Connection successful (Currency: {currency.upper()})",
                    {
                        "status_code": response.status_code,
                        "currency": currency,
                        "account_type": (
                ...
                return (
                    ...
                    {"status_code": response.status_code, "error": "api_error"},
                )

        except requests.exceptions.Timeout:
            return (
                False,
                "Connection timeout - Stripe API unreachable",
                {"error": "timeout"},
            )
            ...
            logger.error(
                "Stripe API key test failed", operation="test_stripe_key", error=str(e)
            )
    ...
    def test_openai_key(self, api_key: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return (
                    True,
                    f"Connection successful ({model_count} models available)",
                    {"status_code": response.status_code, "model_count": model_count},
                )
            elif response.status_code == 401:
                return (
                    False,
            ...
            return (
                False,
                "Connection timeout - OpenAI API unreachable",
                {"error": "timeout"},
            )
        except requests.exceptions.ConnectionError:
            return (
            ...
            logger.error(
                "OpenAI API key test failed", operation="test_openai_key", error=str(e)
            )
            return False, f"Test failed: {str(e)}", {"error": "exception"}
    ...
    def test_airtable_key(self, api_key: str) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    "Airtable API key test successful",
                ...
                return (
                    True,
                    f"Connection successful ({base_count} bases accessible)",
                    {"status_code": response.status_code, "base_count": base_count},
                )
            elif response.status_code == 401:
                return (
                    False,
            ...
            return (
                False,
                "Connection timeout - Airtable API unreachable",
                {"error": "timeout"},
            )
        except requests.exceptions.ConnectionError:
            return (
    ...
    def test_generic_key(
        self, api_key: str, service_type: str
        ...
        logger.info(
            "Generic API key format validation passed",
        ...
        return (
            True,
            f"API key format appears valid for {service_type}",
            {"validation_only": True, "service_type": service_type},
        )
