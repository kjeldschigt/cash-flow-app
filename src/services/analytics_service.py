...
class AnalyticsService:
    ...
    def get_cash_flow_metrics(
        ...
        with self.db.get_connection() as conn:
            ...
            """
            sales_result = conn.execute(sales_query, (start_date.isoformat(), end_date.isoformat())).fetchone()
            ...
            costs_query = """
                ...
                WHERE date BETWEEN ? AND ?
            """
            costs_result = conn.execute(costs_query, (start_date.isoformat(), end_date.isoformat())).fetchone()
            ...
            avg_transaction_size = total_sales / transaction_count if transaction_count > 0 else Decimal('0')
            
            # Calculate growth rates (compare with previous period)
            previous_start = DateUtils.add_months(start_date, -1)
            previous_end = DateUtils.add_months(end_date, -1)
            
            prev_sales_result = conn.execute(sales_query, (previous_start.isoformat(), previous_end.isoformat())).fetchone()
            prev_costs_result = conn.execute(costs_query, (previous_start.isoformat(), previous_end.isoformat())).fetchone()
            
            prev_sales = Decimal(str(prev_sales_result['total_sales'] or 0))
            prev_costs = Decimal(str(prev_costs_result['total_costs'] or 0))
            
            sales_growth = CurrencyUtils.calculate_percentage_change(prev_sales, total_sales)
            cost_growth = CurrencyUtils.calculate_percentage_change(prev_costs, total_costs)
    ...
    def _get_daily_trends(
        ...
        with self.db.get_connection() as conn:
            ...
            sales_results = conn.execute(sales_query, (start_date.isoformat(), end_date.isoformat())).fetchall()
            costs_results = conn.execute(costs_query, (start_date.isoformat(), end_date.isoformat())).fetchall()
    ...
    def get_year_over_year_comparison(self, year: int) -> Dict[str, Any]:
        ...
        current_metrics = self.get_cash_flow_metrics(current_year_start, current_year_end)
        previous_metrics = self.get_cash_flow_metrics(previous_year_start, previous_year_end)
        
        sales_change = CurrencyUtils.calculate_percentage_change(
            previous_metrics.total_sales_usd,
            current_metrics.total_sales_usd
        )
        
        costs_change = CurrencyUtils.calculate_percentage_change(
            previous_metrics.total_costs_usd,
            current_metrics.total_costs_usd
        )
        ...
        return {
            ...
            'changes': {
                ...
                'net_flow_change': current_metrics.net_cash_flow - previous_metrics.net_cash_flow
            }
        }
