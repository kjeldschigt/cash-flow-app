...
class APIKeyService:
    ...
    def add_api_key(self, key_name: str, api_key: str, service_type: str, 
                   added_by_user: str, description: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            is_valid, error_msg = self.encryption.validate_api_key_format(api_key, service_type)
            if not is_valid:
            ...
            try:
                ...
                cursor.execute("SELECT id FROM api_keys WHERE key_name = ? AND is_active = 1", 
                             (key_name,))
                if cursor.fetchone():
    ...
    def get_api_keys(self, service_type: Optional[str] = None, 
            ...
            try:
                ...
                for row in rows:
                    ...
                    api_key_info = APIKeyInfo(
                        ...
                        last_modified=datetime.fromisoformat(row[6]) if row[6] else datetime.now(),
    ...
    def get_api_key_value(self, key_name: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                logger.info("API key value retrieved",
            ...
            logger.error("Failed to retrieve API key value",
                        operation="get_api_key_value",
                        key_name=key_name,
                        error=str(e))
            return None
    
    def update_api_key(self, key_name: str, new_api_key: str, 
        ...
        try:
            ...
            try:
                ...
                is_valid, error_msg = self.encryption.validate_api_key_format(new_api_key, service_type)
                if not is_valid:
                    return False, error_msg
