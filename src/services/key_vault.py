...
class KeyVaultService:
    ...
    def __init__(self, session_id: str, user_id: int):
        ...
        # Database connection
        self.db_path = os.getenv('DATABASE_URL', 'cash_flow_app.db').replace('sqlite:///', '')
        
        logger.info(f"KeyVaultService initialized for session {session_id[:8]}... user {user_id}")
    
    def _get_db_connection(self) -> sqlite3.Connection:
        """Get database connection"""
        return sqlite3.connect(self.db_path)
    
    def _log_audit_event(self, operation: str, key_name: str, success: bool = True, 
    ...
    def store_api_key(self, key_name: str, api_key: str, service_type: str, 
        ...
        try:
            ...
            self._log_audit_event("store_api_key", key_name, True, None, ip_address, user_agent)
            ...
            self._log_audit_event("store_api_key", key_name, False, error_msg, ip_address, user_agent)
    ...
    @contextmanager
    def retrieve_api_key(self, key_name: str, ip_address: Optional[str] = None,
                        user_agent: Optional[str] = None):
        """
        ...
        try:
            ...
            with self._lock:
                if key_name in self._cache:
                    ...
                    if datetime.utcnow() - cached.cached_at < self._cache_timeout:
                        ...
                        self._log_audit_event("retrieve_key_cached", key_name, True, None, ip_address, user_agent)
                        yield APIKeyContext(key_name, cached.key_value, cached.service_type)
                        return
                    else:
            ...
            if not result:
                self._log_audit_event("retrieve_key", key_name, False, "Key not found", ip_address, user_agent)
                yield None
                return
            ...
            self._log_audit_event("retrieve_key", key_name, True, None, ip_address, user_agent)
            yield APIKeyContext(key_name, decrypted_key, service_type)
            
        except Exception as e:
            error_msg = f"Failed to retrieve API key: {str(e)}"
            self._log_audit_event("retrieve_key", key_name, False, error_msg, ip_address, user_agent)
            logger.error(error_msg)
            yield None
    
    def update_api_key(self, key_name: str, new_api_key: str, description: Optional[str] = None,
        ...
        try:
            ...
            with self._get_db_connection() as conn:
                ...
                if cursor.rowcount == 0:
                    ...
                    self._log_audit_event("update_key", key_name, False, error_msg, ip_address, user_agent)
                    return False, error_msg
            ...
            self._log_audit_event("update_key", key_name, True, None, ip_address, user_agent)
    ...
    def delete_api_key(self, key_name: str, ip_address: Optional[str] = None,
                      user_agent: Optional[str] = None) -> Tuple[bool, str]:
        """
        ...
        try:
            with self._get_db_connection() as conn:
                ...
                if cursor.rowcount == 0:
                    ...
                    self._log_audit_event("delete_key", key_name, False, error_msg, ip_address, user_agent)
                    return False, error_msg
            ...
            self._log_audit_event("delete_key", key_name, True, None, ip_address, user_agent)
    ...
    def list_api_keys(self, service_type: Optional[str] = None, 
                     include_inactive: bool = False) -> List[APIKeyInfo]:
        """
        ...
        try:
            ...
            for row in results:
                ...
                api_keys.append(APIKeyInfo(
                    ...
                    last_modified=datetime.fromisoformat(row[6]) if row[6] else datetime.utcnow(),
    ...
    def test_api_key(self, key_name: str, ip_address: Optional[str] = None,
                    user_agent: Optional[str] = None) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            with self.retrieve_api_key(key_name, ip_address, user_agent) as key_context:
                if not key_context:
                    ...
                    self._log_audit_event("test_key", key_name, False, error_msg, ip_address, user_agent)
                    return False, error_msg, {}
                
                # Test the API key
                success, message, details = self.test_service.test_api_key(
                    key_context.key_value, 
                    key_context.service_type
                )
                
                self._log_audit_event("test_key", key_name, success, 
                                    None if success else message, ip_address, user_agent)
...
def clear_session_vault(session_id: str):
    ...
    if session_id in _key_vault_instances:
        try:
            ...
            logger.error(f"Error clearing vault cache for session {session_id[:8]}...: {e}")
