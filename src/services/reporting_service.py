...
class ReportingService:
    ...
    def generate_profit_loss_report(
        self, start_date: date, end_date: date, format_type: str = ReportFormat.JSON
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Calculate totals
            total_revenue = (
                revenue_data["revenue"].sum() if not revenue_data.empty else 0
            )
            ...
            pl_report = {
                ...
                "costs": {
                    ...
                    "by_category": (
                        cost_data.groupby("category")["costs"].sum().to_dict()
                        if not cost_data.empty
                        else {}
                    ),
                    "by_date": (
                        cost_data.to_dict("records") if not cost_data.empty else []
                    ),
                },
                "profitability": {
    ...
    def generate_cash_flow_report(
        ...
        try:
            ...
            operating_cash_flow = (
                cash_flow_data["amount"].sum() if not cash_flow_data.empty else 0
            )
            ...
            cash_flow_report = {
                ...
                "operating_activities": {
                    ...
                    "daily_cash_flow": (
                        daily_cf.to_dict("records") if not daily_cf.empty else []
                    ),
                },
    ...
    def generate_balance_sheet(
        self, as_of_date: date, format_type: str = ReportFormat.JSON
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cash_balance = (
                float(assets_data["cash_balance"].iloc[0])
    ...
    def generate_executive_summary(
        ...
        try:
            ...
            cf_report = self.generate_cash_flow_report(
                start_date, end_date, ReportFormat.JSON
            )
            ...
            operating_cash_flow = cf_report["operating_activities"][
            ...
            executive_summary = {
                ...
                "performance_indicators": {
                    ...
                    "cost_efficiency": (
                        (total_costs / total_revenue * 100) if total_revenue > 0 else 0
                    ),
                    "cash_conversion": (
                        (operating_cash_flow / gross_profit * 100)
                        if gross_profit > 0
                        else 0
                    ),
                },
                "insights": self._generate_insights(
                    total_revenue, total_costs, gross_margin
                ),
                "metadata": {
                    "generated_at": datetime.now().isoformat(),
                    "currency": "USD",
    ...
    def create_visualization(
        self, report_data: Dict[str, Any], chart_type: str = "revenue_trend"
    ) -> go.Figure:
    ...
    def _generate_insights(
        self, revenue: float, costs: float, margin: float
    ) -> List[str]:
        ...
        if margin > 30:
            insights.append(
                "Excellent profit margins indicate strong pricing power and cost control."
            )
        elif margin > 15:
            ...
            insights.append(
                "Negative margins require immediate attention to cost structure."
            )

        cost_ratio = (costs / revenue * 100) if revenue > 0 else 100
        if cost_ratio > 80:
            insights.append(
                "High cost ratio suggests need for operational efficiency improvements."
            )
        elif cost_ratio < 50:
        ...
        if revenue > 0:
            insights.append(
                f"Revenue generation is active with ${revenue:,.2f} in the period."
            )
        else:
            insights.append(
                "No revenue recorded in this period - focus on sales activities."
            )
