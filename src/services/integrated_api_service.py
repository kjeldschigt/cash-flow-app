...
from src.services.api_key_resolver import get_api_key_resolver, ResolvedAPIKey, APIKeySource
...
class IntegratedAPIService:
    ...
    def get_stripe_client(self):
        ...
        try:
            ...
            yield None, {"error": "Stripe library not installed. Run: pip install stripe"}
            return
    ...
    def get_openai_client(self):
        ...
        try:
            ...
            yield None, {"error": "OpenAI library not installed. Run: pip install openai"}
            return
    ...
    def get_airtable_client(self):
        ...
        try:
            ...
            yield None, {"error": "Requests library not installed. Run: pip install requests"}
            return
    ...
    def get_sendgrid_client(self):
        ...
        try:
            ...
            yield None, {"error": "SendGrid library not installed. Run: pip install sendgrid"}
            return
    ...
    def get_service_status(self, service_type: str) -> Dict[str, Any]:
        ...
        """
        resolved_key = self.resolver.resolve_api_key(f"{service_type}_api_key", service_type)
    ...
    def get_all_service_statuses(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        services = ['stripe', 'openai', 'airtable', 'twilio', 'sendgrid', 'aws', 'google_cloud', 'azure']
        statuses = {}
    ...
    def validate_service_configuration(self, service_type: str) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            if service_type == "stripe":
                ...
                if resolved_key.is_valid:
                    return True, f"{service_type.title()} API key found", {
                        "source": resolved_key.source.value,
                        "masked_key": resolved_key.masked_value
                    }
                else:
                    return False, resolved_key.error_message or "API key not found", {
                        "source": resolved_key.source.value
                    }
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "configured_services": len([s for s in statuses.values() if s["is_configured"]]),
            "source_breakdown": {},
            "services": statuses
        }
...
def clear_api_service_cache():
    ...
    if _api_service_instance:
        _api_service_instance.refresh_service_cache()
