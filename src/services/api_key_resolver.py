    ...
    def __post_init__(self):
        ...
        if self.key_value:
            try:
                ...
                self.masked_value = f"****{self.key_value[-4:] if len(self.key_value) > 4 else '****'}"
                self.is_valid = True
        else:
...
class APIKeyResolver:
    ...
    def _check_environment_source(self, service_type: str) -> Optional[str]:
        ...
        for env_var in env_vars:
            ...
            if value and value.strip():
                logger.debug(f"Found {service_type} key in environment variable {env_var}")
                return value.strip()
        ...
        for secret_key in secret_keys:
            try:
                ...
                if value and str(value).strip():
                    logger.debug(f"Found {service_type} key in Streamlit secrets {secret_key}")
    ...
    def resolve_api_key(self, key_name: str, service_type: str, use_cache: bool = True) -> ResolvedAPIKey:
        """
    ...
    def get_all_resolved_keys(self, service_types: list = None) -> Dict[str, ResolvedAPIKey]:
        """
    ...
    def get_source_priority_info(self) -> Dict[str, Any]:
        ...
        return {
            "priority_order": [
                {"source": "Database (UI)", "priority": 1, "description": "Keys stored via Settings UI"},
                {"source": "Environment Variables", "priority": 2, "description": "System environment variables"},
                {"source": "Streamlit Secrets", "priority": 3, "description": "Streamlit Cloud secrets"}
            ],
    ...
    def invalidate_cache(self, key_name: str = None, service_type: str = None):
        ...
        if key_name and service_type:
            ...
            for cache_key in cache.keys():
                if (key_name and key_name in cache_key) or (service_type and service_type in cache_key):
...
def clear_resolver_cache(session_id: str, user_id: int = None):
    ...
    if user_id:
        ...
        keys_to_remove = [k for k in _resolver_instances.keys() if k.startswith(f"{session_id}_")]
        for key in keys_to_remove:
