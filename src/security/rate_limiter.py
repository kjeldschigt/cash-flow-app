...
class RedisRateLimiter:
    ...
    def __init__(self, redis_client: redis.Redis):
        ...
        self.default_rules = {
            'auth_login': RateLimitRule(5, 300, RateLimitStrategy.SLIDING_WINDOW, 900),  # 5 attempts per 5 min, block 15 min
            'auth_register': RateLimitRule(3, 3600, RateLimitStrategy.FIXED_WINDOW, 3600),  # 3 attempts per hour
            'password_reset': RateLimitRule(3, 3600, RateLimitStrategy.SLIDING_WINDOW, 3600),  # 3 attempts per hour
            'api_call': RateLimitRule(100, 60, RateLimitStrategy.TOKEN_BUCKET, 60),  # 100 calls per minute
            'session_create': RateLimitRule(10, 300, RateLimitStrategy.SLIDING_WINDOW, 600),  # 10 sessions per 5 min
        }
    ...
    def check_rate_limit(self, identifier: str, rule_name: str, 
        ...
        try:
            ...
            if self.redis_client.exists(block_key):
                ...
                logger.info("Request blocked by rate limiter",
                           rule_name=rule_name,
            ...
            if rule.strategy == RateLimitStrategy.SLIDING_WINDOW:
                ...
                logger.error("Unknown rate limit strategy", strategy=rule.strategy.value)
    ...
    def _sliding_window_check(self, identifier: str, rule_name: str, 
    ...
    def _fixed_window_check(self, identifier: str, rule_name: str, 
                           rule: RateLimitRule) -> RateLimitResult:
    ...
    def _token_bucket_check(self, identifier: str, rule_name: str, 
                           rule: RateLimitRule) -> RateLimitResult:
    ...
    def _block_identifier(self, identifier: str, rule_name: str, duration: int):
        ...
        logger.warning("Identifier blocked due to rate limit exceeded",
                      rule_name=rule_name,
                      duration=duration,
    ...
    def add_custom_rule(self, rule_name: str, rule: RateLimitRule):
        ...
        logger.info("Custom rate limit rule added",
                   rule_name=rule_name,
                   max_attempts=rule.max_attempts,
                   window_seconds=rule.window_seconds)
    ...
    def get_blocked_identifiers(self, rule_name: str) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error("Failed to get blocked identifiers",
                        rule_name=rule_name,
                        error_type=type(e).__name__,
