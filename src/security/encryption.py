...
class DataEncryption:
    ...
    def _validate_master_key(self):
        ...
        if len(self.master_key) < 32:
            raise ValueError("Master key must be at least 32 characters long for security")
        
        environment = os.getenv('ENVIRONMENT', 'development')
        if environment == 'production' and not os.getenv('ENCRYPTION_MASTER_KEY'):
            raise ValueError("Master key must be provided via ENCRYPTION_MASTER_KEY in production")
...
class SecureStorage:
    ...
    def store_sensitive_data(self, key: str, data: Union[str, Dict[str, Any]]) -> str:
        ...
        try:
            ...
            logger.error("Error storing sensitive data", 
                        error_type=type(e).__name__,
                        operation="store_sensitive_data")
            raise
    ...
    def retrieve_sensitive_data(self, encrypted_data: str, as_dict: bool = False) -> Union[str, Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Error retrieving sensitive data", 
                        error_type=type(e).__name__,
