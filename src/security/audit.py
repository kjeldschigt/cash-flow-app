...
class AuditLogger:
    ...
    def __init__(self, db_connection: DatabaseConnection, secure_storage: Optional[SecureStorage] = None):
        ...
        try:
            ...
            if details and self._contains_sensitive_data(details):
                audit_entry['details'] = self.secure_storage.store_sensitive_data(
                    f"audit_{entity_id}_{datetime.now().timestamp()}", 
                    details
                )
            ...
            with self.db.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if audit_entry.get('details_encrypted'):
                        try:
                            audit_entry['details'] = self.secure_storage.retrieve_sensitive_data(
                                audit_entry['details'], as_dict=True
                            )
                        except Exception as e:
                            self.logger.error(f"Failed to decrypt audit details: {str(e)}")
    ...
    def _store_audit_entry(self, audit_entry: Dict[str, Any]) -> None:
        ...
        with self.db.get_connection() as conn:
            ...
            ''', (
                ...
                audit_entry['level'],
                json.dumps(audit_entry['details']) if not audit_entry.get('details_encrypted') else audit_entry['details'],
    ...
    def _mask_audit_entry(self, audit_entry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if '@' in masked_entry.get('user_id', ''):
            masked_entry['user_id'] = self.secure_storage.mask_pii_in_logs(masked_entry['user_id'])
