...
class SecureCookieManager:
    ...
    def __init__(self, secret_key: str = None, domain: str = None, secure: bool = None):
        self.secret_key = secret_key or os.getenv('COOKIE_SECRET_KEY') or os.getenv('SESSION_SECRET_KEY')
        if not self.secret_key:
            raise ValueError("COOKIE_SECRET_KEY or SESSION_SECRET_KEY environment variable is required")
        
        self.domain = domain or os.getenv('COOKIE_DOMAIN')
        self.secure = secure if secure is not None else (os.getenv('ENVIRONMENT', 'development') == 'production')
        ...
        logger.info("Secure cookie manager initialized",
                   secure=self.secure,
    ...
    def set_csrf_cookie(self, csrf_token: str, max_age: int = None) -> str:
        ...
        try:
            ...
            logger.error("Failed to set CSRF cookie",
                        error_type=type(e).__name__,
                        operation="set_csrf_cookie")
            return ""
    ...
    def clear_session_cookies(self) -> str:
        ...
        try:
            ...
            logger.error("Failed to clear session cookies",
                        error_type=type(e).__name__,
    ...
    def _get_cookies_from_request(self) -> Dict[str, str]:
        ...
        try:
            ...
            logger.debug("Could not extract cookies from request",
                        error_type=type(e).__name__,
    ...
    def validate_csrf_from_form(self, form_data: Dict[str, Any], 
                               session_csrf_token: str) -> bool:
        """
    ...
    def generate_secure_headers(self) -> Dict[str, str]:
        ...
        return {
            ...
            'Strict-Transport-Security': 'max-age=31536000; includeSubDomains' if self.secure else '',
