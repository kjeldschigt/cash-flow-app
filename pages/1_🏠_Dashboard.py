...
try:
    ...
    UIComponents.section_header(
        "Key Metrics", f"Performance overview for {range_select.lower()}"
    )
    ...
    if cash_flow_metrics:
        ...
        with col4:
            UIComponents.metric_card(
                "Transactions", str(cash_flow_metrics.transaction_count), "total"
            )
        ...
        # Charts Section
        UIComponents.section_header(
            "Analytics Charts", "Visual insights into cash flow trends"
        )
        ...
        if monthly_summary:
            ...
            with col1:
                ChartComponents.monthly_trend_chart(
                    monthly_summary, title="Monthly Revenue vs Costs"
                )

            with col2:
                ChartComponents.category_breakdown_chart(
                    monthly_summary, title="Cost Breakdown by Category"
                )
