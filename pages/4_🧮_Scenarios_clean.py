...
from services.fx import get_rate_scenarios, get_monthly_rate
...
try:
    ...
    with st.sidebar:
        ...
        if economy_scenario == "Recession":
            ...
            economy_ads_multiplier = 1.2 if macro_data['us_consumer_confidence'] < 60 else 1.0
        ...
        google_ads_multiplier = st.slider("Google Ads", 0.5, 3.0, economy_ads_multiplier, step=0.1)
    ...
    base_metrics = [
        {"title": "Avg Monthly Sales", "value": f"${avg_monthly_sales:,.0f}", "caption": "Historical average"},
        {"title": "Avg Monthly Costs", "value": f"${avg_monthly_costs:,.0f}", "caption": "Historical average"},
        {"title": "Net Monthly", "value": f"${avg_monthly_sales - avg_monthly_costs:,.0f}", "caption": "Monthly profit"},
        {"title": "Consumer Confidence", "value": f"{macro_data['us_consumer_confidence']}", "caption": "Economic indicator"}
    ]
    ...
    for year in range(1, years + 1):
        ...
        total_annual_costs = costa_usd_annual + costa_crc_usd_annual + hk_usd_annual + google_ads_annual
    ...
    show_error("Scenario Analysis Error", f"An error occurred while loading the scenario analysis: {str(e)}")
