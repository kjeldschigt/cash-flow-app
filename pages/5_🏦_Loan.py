...
try:
    ...
    original_total = loan_summary["original_principal"] + (
        ANNUAL_INTEREST * LOAN_TERM_YEARS
    )
    ...
    status_metrics = [
        {
            ...
            "value": f"${loan.outstanding:,.0f}",
            "caption": "Remaining principal",
        },
        {
            "title": "Total Payments",
            "value": f"${loan.get_total_payments():,.0f}",
            "caption": f"{loan_summary['payment_count']} payments made",
        },
        {
    ...
    with st.spinner("Loading progress visualization..."):
        ...
        st.caption(
            f"Total loan value: ${original_total:,.0f} | Paid: ${paid_amount:,.0f} | Remaining: ${remaining_amount:,.0f}"
        )
